...
> **Post-launch fix pass (2026-07-22):** after deploying to Netlify, a visual QA pass (Playwright screenshots + direct pixel review of every source file) found that the original photo audit had mislabeled several photos: 3 "portrait" entries and the "commercial" photo were actually love-story content, and 7 love-story alt-text descriptions didn't match their images (all now corrected — see `data/photos.ts`, category counts are now fashion-night 20 / love-story 32 / portrait 11 / commercial 0). Two source photos also turned out to be physically rotated 90° with no EXIF flag; `scripts/copy-photos.mjs` now corrects this via `sharp` on copy. Also fixed in the same pass: the Services grid (simplified to 5 uniform cards once the phantom commercial photo was removed), dead space in Hero/About (`items-center` → `items-start`), and a missing mobile navigation menu. See the git log around commit `283333d` for the full diff and reasoning; the design spec has been updated to match.
